...
func main() {
	...
	def busca_viacep(cep):
  	...
  	if thread1.is_alive():
    	...
    	thread1.join()
  		...
  		resultado2 = thread2.get_resultado()
	...
	def exibir_resultado(resultado):
	  if resultado is not None:
    	...
    	api = resultado[1]
    	printPrintf("Endereço: {endereco['logradouro']}, {endereco['bairro']}, {endereco['localidade']} - {endereco['uf']}")
    	printPrintf("API: {api}")
  	else:
    	print("Erro: Timeout na busca  em ambas as APIs")


		cep = input("Digite o CEP: ")
result = busca_cep(cep)
